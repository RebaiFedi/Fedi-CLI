...
import { TO_SONNET_PATTERN, TO_CODEX_PATTERN } from './types.js';

test('relay patterns match only standalone directive lines', () => {
  const directToClaude = '  [TO:SONNET] endpoint /api/tasks is ready  ';
  const directToCodex = '[TO:CODEX] need POST /api/login with JWT';

  const claudeMatch = directToClaude.match(TO_SONNET_PATTERN);
...
test('relay patterns ignore mentions inside normal sentences', () => {
  const mentionToClaude = 'Use [TO:SONNET] message format when you need to relay.';
  const mentionToCodex = '- To send a message, write [TO:CODEX] your message here';

  assert.equal(mentionToClaude.match(TO_SONNET_PATTERN), null);
...
test('relay patterns allow empty content (Codex puts content on next line) and require exact casing', () => {
  ...
  const claudeAlone = '[TO:SONNET]'.match(TO_SONNET_PATTERN);
  assert.ok(claudeAlone, '[TO:SONNET] alone should match');
  ...
  assert.equal('[to:sonnet] hello'.match(TO_SONNET_PATTERN), null);
